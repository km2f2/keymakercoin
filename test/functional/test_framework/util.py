...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "keymaker.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "keymaker.conf")):
        with open(os.path.join(datadir, "keymaker.conf"), 'r', encoding='utf8') as f:
            for line in f:
