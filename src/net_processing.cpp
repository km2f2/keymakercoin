...
namespace {
    ...
    /** Keymakercoin: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
void static ProcessGetBlockData(CNode* pfrom, const Consensus::Params& consensusParams, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (inv.hash == pfrom->hashContinue)
        {
            ...
            // Keymakercoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        // Keymakercoin: relay sync-checkpoint
        {
            LOCK(cs_main);
...
#ifdef ENABLE_CHECKPOINTS
        // Keymakercoin: ask for pending sync-checkpoint if any
    ...
    // Keymakercoin: set/unset network serialization mode for new clients
  ...
  //      }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            // Keymakercoin: quick check to see if we should ban peers for PoS spam
    ...
    //            }
            ...
            // Keymakercoin: store in memory until we can connect it to some chain
        ...
        // Keymakercoin: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
            LOCK(cs_main);
            // Keymakercoin: try to select next block in a constant time
