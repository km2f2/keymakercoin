...
private:
    ...
    bool SelectCoinsForStaking(unsigned int nSpendTime, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const;
...
public:
    ...
    bool CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, CMutableTransaction &txNew);
