...
// keymaker: optional setting to unlock wallet for block staking only;
...
//    return n ? n : nTimeReceived;
    ...
    // keymaker: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// keymaker: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // keymaker: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // keymaker: set time
...
//                }
                ...
                // keymaker: sub-cent change is moved to fee
...
// keymaker: create coin stake transaction
