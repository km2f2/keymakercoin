...
const std::string strMessageMagic = "Keymaker Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // keymaker: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
int64_t GetProofOfWorkReward(unsigned int nHeight) {
   ...
   if (nHeight == 1) { return 6000000 * COIN; } // Premine
	    ...
	    else if (nHeight > 1576800 && nHeight <= 2102400) {return  2.5 * COIN;}
		else if (nHeight > 2102400 && nHeight <= 2628000) {return  1.25 * COIN;}
		else if (nHeight > 2628000 && nHeight <= 3153600) {return  0.625 * COIN;}
		else if (nHeight > 3153600 && nHeight <= 3679200) {return  0.3125 * COIN;}
        else if (nHeight > 3679200 && nHeight <= 4204800) {return  0.15625 * COIN;}	
	    else if (nHeight > 4204800 && nHeight <= 4730400) {return  0.078125 * COIN;}
		else if (nHeight > 4730400 && nHeight <= 5256000) {return  0.03906250 * COIN;}
		else if (nHeight > 5256000 && nHeight <= 5781600) {return  0.01953125 * COIN;}
		else if (nHeight > 5781600 && nHeight <= 6307200) {return  0.00976563 * COIN;}
   		else if (nHeight > 6307200 && nHeight <= 6832800) {return  0.00488281 * COIN;}
        else if (nHeight > 6832800 && nHeight <= 7358400) {return  0.00244141 * COIN;}
        else if (nHeight > 7358400 && nHeight <= 7884000) {return  0.00122070 * COIN;}
        else if (nHeight > 7884000 && nHeight <= 8409600) {return  0.00061035 * COIN;}
        else if (nHeight > 8409600 && nHeight <= 8935200) {return  0.00030518 * COIN;}
        else if (nHeight > 8935200 && nHeight <= 9460800) {return  0.00015259 * COIN;}
        else if (nHeight > 9460800 && nHeight <= 9986400) {return  0.00007629 * COIN;}
        else if (nHeight > 9986400 && nHeight <= 10512000) {return  0.00003815 * COIN;}
        else if (nHeight > 10512000 && nHeight <= 11037600) {return  0.00001907 * COIN;}
        else if (nHeight > 11037600 && nHeight <= 11563200) {return  0.00000954 * COIN;}
        else if (nHeight > 11563200 && nHeight <= 12088800) {return  0.00000477 * COIN;}
        else if (nHeight > 12088800 && nHeight <= 12614400) {return  0.00000238 * COIN;}
        else if (nHeight > 12614400 && nHeight <= 13140000) {return  0.00000119 * COIN;}
        else if (nHeight > 13140000 && nHeight <= 13665600) {return  0.00000060 * COIN;}
        else if (nHeight > 13665600 && nHeight <= 14191200) {return  0.00000030 * COIN;}
        else if (nHeight > 14191200 && nHeight <= 14716800) {return  0.00000015 * COIN;}
        else if (nHeight > 14716800 && nHeight <= 15242400) {return  0.00000007 * COIN;}
        else if (nHeight > 15242400 && nHeight <= 15768000) {return  0.00000004 * COIN;}
        else if (nHeight > 15768000 && nHeight <= 16293600) {return  0.00000002 * COIN;} 
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // keymaker: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // keymaker
            undo.nTime = alternate.nTime;           // keymaker
        } else {
...
bool KeymakerContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    uint256 hashProofOfStake = uint256();
    uint256 targetProofOfStake = uint256();
    // keymaker: verify hash target and signature of coinstake tx
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!KeymakerContextualBlockChecks(block, state, pindex, fJustCheck))
  ...
  //  if (block.IsProofOfWork() && (pindex->nHeight > 0 && pindex->pprev->nPOWBlockHeight + 1 > params.nTotalPOWBlock))
    ...
    // keymaker: coinbase reward check relocated to CheckBlock()
    ...
    // keymaker: track money supply and mint amount info
    ...
    // keymaker: fees are not collected by miners as in bitcoin
    // keymaker: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature) {
    ...
    // keymaker: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // keymaker: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // keymaker: check transaction timestamp
    ...
    // keymaker: check block signature
...
#ifdef ENABLE_CHECKPOINTS
    // keymaker: check that the block satisfies synchronized checkpoint
...
//    // keymaker: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // keymaker: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // keymaker: check PoS
    if (fCheckPoS && !KeymakerContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // keymaker: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // keymaker: if responsible for sync-checkpoint send it
...
#ifdef ENABLE_CHECKPOINTS
    // keymaker: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams &chainparams) {
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // keymaker: txindex is always enabled
...
// keymaker: total coin age spent in transaction, in the unit of coin-days.
