 ...
 * between keymakerd, and keymaker-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    StopHTTPServer();
#ifdef ENABLE_WALLET
    GenerateKeymaker(false, 0, Params());
    FlushWallets();
#endif
    MapPort(false);
...
/** Sanity checks
 *  Ensure that Keymaker is running in a usable environment with all
...
{
    // Make sure only a single Keymaker process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // keymaker: init hash seed
    keymakerRandseed = GetRand(1 << 30);

#ifdef ENABLE_CHECKPOINTS
    // keymaker: moved here because ECC need to be initialized to execute this
    if (gArgs.IsArgSet("-checkpointkey")) // keymaker: checkpoint master priv key
    {
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if keymaker is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if keymaker is started while in a temporary directory.\n",
...
#ifdef ENABLE_CHECKPOINTS
                // keymaker: initialize synchronized checkpoint
                if (!fReindex && !WriteSyncCheckpoint(chainparams.GenesisBlock().GetHash()))
                    return error("LoadBlockIndex() : failed to init sync checkpoint");

                // keymaker: if checkpoint master key changed must reset sync-checkpoint
    ...
    GenerateKeymaker(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), gArgs.GetArg("-gen", DEFAULT_GENERATE_THREADS), chainparams);
#endif
