...
install -p %{SOURCE100} %{buildroot}%{_datadir}/pixmaps/keymaker.png
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5996
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5995
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5997
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5994
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5998
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5993
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 5996
	%{_sbindir}/semanage port -d -p tcp 5995
	%{_sbindir}/semanage port -d -p tcp 5997
	%{_sbindir}/semanage port -d -p tcp 5994
	%{_sbindir}/semanage port -d -p tcp 5998
	%{_sbindir}/semanage port -d -p tcp 5993
	for selinuxvariant in %{selinux_variants}; do
